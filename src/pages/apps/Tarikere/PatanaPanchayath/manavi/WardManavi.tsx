...
// Reuse the same CSS as VillageManavi — import from parent Tarikere folder
import "../../index.css";
...
export default function WardManavi() {
  ...
  const handleExcelDownload = useCallback(() => {
    ...
    saveAs(
      ...
      `${ward?.name || "Ward"}_ಮನವಿ_List.xlsx`
    );
  }, [filteredList, ward]);
  ...
  const handlePdfDownload = useCallback(async () => {
    ...
    const element = document.getElementById("ward-manavi-pdf-area");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 8 }}>
      ...
      <div className="cw-header">
        <h4 className="cw-title">
          {ward?.name
            ? <><span className="cw-title-village">{ward.name}</span>{" "}ವಾರ್ಡ್ ಮನವಿಗಳು</>
            : "ಮನವಿಗಳು"}
        </h4>
      ...
      {/* ── TABLE ── */}
      <div
        id="ward-manavi-pdf-area"
        className={isPdf ? "pdf-mode" : "cw-scroll-wrap"}
      >
        {isPdf && (
          <div className="pdf-title">
            <h2>{ward?.name || ""} ವಾರ್ಡ್ ಮನವಿಗಳು</h2>
      ...
      <DeleteConfirmModal
        ...
        onConfirm={() => {
          if (deleteId && wardId) dispatch(deleteWardmanavi(deleteId, wardId));
        }}
      />
    </div>
  );
